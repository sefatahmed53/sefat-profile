...
import { ArrowDown, Mail, ArrowUpRight, Award, FileText, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function Hero({ profile, onTabChange }: HeroProps) {
  ...
  const slideImages = profile.photoSlides ?? [
  ...
  return (
    <section className="relative flex min-h-[92vh] items-center justify-center overflow-hidden bg-[#0A0A0A] px-4 pt-10 sm:px-6 lg:px-8" id="hero-section">
      ...
      >
        ...
        <motion.div
          variants={itemVariants}
          className="mt-10 relative mx-auto max-w-4xl overflow-hidden rounded-[32px] border border-zinc-800 bg-[#080a0f]/80 shadow-[0_30px_80px_rgba(0,0,0,0.35)]"
        >
          <img
            src={slideImages[slideIndex]}
            alt={`Profile slide ${slideIndex + 1}`}
            className="h-[360px] w-full object-cover transition-all duration-700 ease-out"
          />
